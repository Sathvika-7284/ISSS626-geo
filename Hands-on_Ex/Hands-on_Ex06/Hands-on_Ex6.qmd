...
write_rds(shan_ict, "data/rds/shan_sf.rds")
```

## 5.2 Min-Max standardisation
...
## 5.4 Visualising the standardised clustering variables
...
## 5.6 Computing hierarchical clustering
...
## 5.10 Visually-driven hierarchical clustering analysis
...
## 8.3 Cluster-wise summary statistics for all ICT variables

```{r}
cluster_summary <- shan_sf_ngeo_cluster %>% 
  st_set_geometry(NULL) %>%
  group_by(CLUSTER) %>%
  summarise(
    mean_RADIO_PR = mean(RADIO_PR, na.rm = TRUE),
    sd_RADIO_PR = sd(RADIO_PR, na.rm = TRUE),
    mean_TV_PR = mean(TV_PR, na.rm = TRUE),
    sd_TV_PR = sd(TV_PR, na.rm = TRUE),
    mean_LLPHONE_PR = mean(LLPHONE_PR, na.rm = TRUE),
    sd_LLPHONE_PR = sd(LLPHONE_PR, na.rm = TRUE),
    mean_MPHONE_PR = mean(MPHONE_PR, na.rm = TRUE),
    sd_MPHONE_PR = sd(MPHONE_PR, na.rm = TRUE),
    mean_COMPUTER_PR = mean(COMPUTER_PR, na.rm = TRUE),
    sd_COMPUTER_PR = sd(COMPUTER_PR, na.rm = TRUE),
    mean_INTERNET_PR = mean(INTERNET_PR, na.rm = TRUE),
    sd_INTERNET_PR = sd(INTERNET_PR, na.rm = TRUE),
    n = n()
  )
