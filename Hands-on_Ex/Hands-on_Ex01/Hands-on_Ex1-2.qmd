...
#### Creating centroid points for each planning subzone

```{r}

# Temporarily disabling s2 due to invalid polygons
sf::sf_use_s2(FALSE)

# Fixing invalid geometries before centroid calculation
mpsz_valid <- mpsz_pop2024 %>%
  st_make_valid()

# Computing centroids
mpsz_centroids <- mpsz_valid %>%
  st_centroid() %>%
  mutate(
